...
void setup() {
  ...
  Serial.println("\n=========================================");
  Serial.println("   K-2SO Professional Controller v1.0.4");
  Serial.println("     Droid Logic Motion Board v1.2");
...
void initializeHardware() {
  ...
  while (attempts < 5 && fileCount == 0) {
    mp3.loop();
    delay(200);
    fileCount = mp3.getTotalTrackCount();
    attempts++;
    if (fileCount == 0) {
      Serial.printf("- DFPlayer attempt %d: No files detected\n", attempts);
    }
  }
