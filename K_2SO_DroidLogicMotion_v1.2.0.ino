...
VISUAL SYSTEM:
• Dual NeoPixel eyes - configurable 7-LED or 13-LED per eye (default: 13-LED)
  - 13-LED version: 1 center LED + 12-LED ring for advanced animations
  - 7-LED version: standard strip (LEDs 0-6)
• 12 advanced animation modes: Solid, Flicker, Pulse, Scanner, Iris, Targeting,
  Ring Scanner, Spiral, Focus, Radar, Heartbeat, Alarm
• Mode-specific eye colors (Ice Blue/Red/Amber)
• WS2812 Detail LED strip system (1-8 LEDs, default 5 LEDs)
  - 5 animation patterns: Blink, Fade, Chase, Pulse, Random
• Status LED system - single WS2812 LED for system status indication
• Brightness control (0-255 levels) for all LED systems
...
• Professional Web Interface (mobile responsive, HTTP authentication)
• Voice Assistant Control (Google Home, Alexa, Siri via IFTTT/Shortcuts)
  - 10 voice trigger commands for hands-free operation
• Advanced Serial CLI (115200 baud configuration)
  - WiFi configuration via serial commands (v1.2.0+)
...
NETWORK:         wifi set <ssid> <password>, wifi show, wifi reconnect
                 ap set <ssid> <password>, ap enable, ap start, ap show
CALIBRATION:     servo eye center 90 90, led brightness 200, sound volume 25
LED CONTROL:     led mode [solid|flicker|pulse|scanner|iris|targeting|...]
                 detail count 5, detail pattern [blink|fade|chase|pulse|random]
...
• HTTP Basic Authentication for security (configurable in config.h)
• Live servo positioning gamepad with 8 directional presets + center
• Eye LED control: 5 color presets, 2 animation modes, brightness slider
• Detail LED control: LED count slider (1-8), 5 patterns, brightness control
• Audio management: Volume slider, 8 quick-play sound buttons
• Behavior mode selector: Scanning/Alert/Idle mode switching
• System status dashboard: Mode, uptime, free RAM (auto-refresh)
• Mobile-optimized responsive design with glassmorphism UI
...
ESP32-S3 PINS:
• GP3  → Left Eye NeoPixel (13 LEDs default, configurable to 7 LEDs)
• GP4  → Right Eye NeoPixel (13 LEDs default, configurable to 7 LEDs)
• GP5  → Eye Pan Servo
• GP6  → Eye Tilt Servo
• GP7  → Head Pan Servo
• GP8  → Head Tilt Servo
• GP9  → IR Receiver (TSOP38238)
• GP10 → Detail LED Strip (WS2812, 1-8 LEDs, default 5 LEDs)
• GP11 → DFPlayer TX
• GP12 → DFPlayer RX
...
// Note: IR Library (IRremote.hpp) is included in handlers.cpp where it's used
// This avoids global namespace pollution while maintaining functionality
...
void setup() {
  ...
  Serial.println(F("\n========================================="));
  Serial.println(F("   K-2SO Professional Controller v1.2.0"));
