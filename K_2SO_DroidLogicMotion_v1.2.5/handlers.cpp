...
void processCommand(String fullCommand) {
  ...
  switch (command) {
    ...
    case CMD_CLEAR: {
      ...
      unsigned long clearTimeout = millis();
      while (!Serial.available()) {
        delay(10);
        if (millis() - clearTimeout > 30000) {
          Serial.println(F("\n⏱️ Timeout (30s) - operation cancelled. System ready for new commands."));
          break;
        }
      }
      if (Serial.available()) {
...
void handleWiFiCommand(String params) {
  ...
  if (subCmd == "show") {
    ...
    unsigned long wifiResetTimeout = millis();
    while (!Serial.available()) {
      delay(10);
      if (millis() - wifiResetTimeout > 30000) {
        Serial.println(F("\n⏱️ Timeout (30s) - operation cancelled. System ready for new commands."));
        return;
      }
    }
...
void handleAPCommand(String params) {
  ...
  if (subCmd == "show") {
    ...
    unsigned long apResetTimeout = millis();
    while (!Serial.available()) {
      delay(10);
      if (millis() - apResetTimeout > 30000) {
        Serial.println(F("\n⏱️ Timeout (30s) - operation cancelled. System ready for new commands."));
        return;
      }
    }
...
void clearAllData() {
  ...
  unsigned long clearAllTimeout = millis();
  while (!Serial.available()) {
    delay(10);
    if (millis() - clearAllTimeout > 30000) {
      Serial.println(F("\n⏱️ Timeout (30s) - operation cancelled. System ready for new commands."));
      return;
    }
  }
...
void restoreFromSerial() {
  ...
  unsigned long restoreTimeout = millis();
  while (!Serial.available()) {
    delay(10);
    if (millis() - restoreTimeout > 30000) {
      Serial.println(F("\n⏱️ Timeout (30s) - operation cancelled. System ready for new commands."));
      return;
    }
  }
  ...
  // Non-blocking data collection with timeout
  String hexData = "";
  ...
  while (millis() - dataTimeout < DATA_TIMEOUT_MS) {
    if (Serial.available()) {
      ...
      if (c == '\n' || c == '\r') {
        if (hexData.length() > 0) {
          break;  // Data received and newline detected
        }
      } else {
        hexData += c;
        dataTimeout = millis();  // Reset timeout on data received
      }
    }
  ...
  for (size_t i = 0; i < sizeof(config) * 2; i++) {
    ...
    if (!isxdigit(c)) {
      Serial.printf("Error: Invalid hex character '%c' at position %d\n", c, i);
      return;
    }
  }
  ...
  if (tempConfig.magic == (uint8_t)EEPROM_MAGIC && tempConfig.version == 1) {
    ...
    Serial.printf("  Magic: 0x%02X (expected 0x%02X)\n", tempConfig.magic, EEPROM_MAGIC);
    Serial.printf("  Version: %d (expected 1)\n", tempConfig.version);
  }
}
...
void handleSeqMapList() {
  ...
  // Build JSON response with IR mappings
