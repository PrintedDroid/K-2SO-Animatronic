...
struct __attribute__((packed)) IRButton {
  ...
  uint32_t code;
  bool isConfigured;
  char sequenceName[32];  // Name of sequence to trigger (empty = standard action)
};
...
// Legacy v1 configuration structures for EEPROM migration only.
// These mirror the v1.2.3/v1.2.4 layout before IRButton gained sequenceName[32].
struct __attribute__((packed)) IRButtonV1 {
  char name[16];
  uint32_t code;
  bool isConfigured;
};
...
static_assert(sizeof(ConfigData) <= EEPROM_SIZE, "ConfigData exceeds EEPROM_SIZE");
...
enum Command {
  ...
  CMD_SEQ      // Sequence recording and playback command
};
