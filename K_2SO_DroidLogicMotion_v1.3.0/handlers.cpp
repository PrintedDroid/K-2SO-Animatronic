...
#include "sequences.h"    // Sequence recording and playback
#include "webpage.h"
#include "globals.h"
#include "Mp3Notify.h"    
...
bool safeParseInt(const String& str, int& value, int minVal = INT_MIN, int maxVal = INT_MAX) {
  if (str.length() == 0) {
    return false;
  }
  ...
  for (int i = startIdx; i < (int)str.length(); i++) {
    if (!isDigit(str.charAt(i))) {
      return false;  // Non-digit character found
    }
  }

  long parsed = str.toInt();
...
static bool waitForSerialWithTimeout(unsigned long timeoutMs) {
...
static uint32_t calculateChecksumForConfig(const ConfigData& cfg) {
  uint32_t sum = 0;
...
static uint32_t calculateChecksumForConfigV1(const ConfigDataV1& cfg) {
  uint32_t sum = 0;
  const uint8_t* data = reinterpret_cast<const uint8_t*>(&cfg);
  for (size_t i = 0; i < sizeof(cfg) - sizeof(uint32_t); i++) {
    sum += data[i];
...
static void setDefaultConfiguration() {
  ...
  config.eyeVersion = EYE_VERSION_13LED;
...
static void applyLegacyConfigV1ToCurrent(const ConfigDataV1& oldConfig) {
  memset(&config, 0, sizeof(config));
  config.magic = (uint8_t)EEPROM_MAGIC;
  config.version = 2;
  config.writeCount = oldConfig.writeCount;

  config.remoteType = oldConfig.remoteType;
  config.buttonCount = oldConfig.buttonCount;
  config.currentProfile = oldConfig.currentProfile;
  config.savedMode = oldConfig.savedMode;
  config.savedVolume = oldConfig.savedVolume;
  config.irEnabled = oldConfig.irEnabled;

  config.eyePanCenter = oldConfig.eyePanCenter;
  config.eyeTiltCenter = oldConfig.eyeTiltCenter;
  config.eyePanMin = oldConfig.eyePanMin;
  config.eyePanMax = oldConfig.eyePanMax;
  config.eyeTiltMin = oldConfig.eyeTiltMin;
  config.eyeTiltMax = oldConfig.eyeTiltMax;

  config.headPanCenter = oldConfig.headPanCenter;
  config.headTiltCenter = oldConfig.headTiltCenter;
  config.headPanMin = oldConfig.headPanMin;
  config.headPanMax = oldConfig.headPanMax;
  config.headTiltMin = oldConfig.headTiltMin;
  config.headTiltMax = oldConfig.headTiltMax;
  ...
  config.wifiConfigured = oldConfig.wifiConfigured;

  memcpy(config.apSSID, oldConfig.apSSID, sizeof(config.apSSID));
  memcpy(config.apPassword, oldConfig.apPassword, sizeof(config.apPassword));
  config.apConfigured = oldConfig.apConfigured;
  config.apEnabled = oldConfig.apEnabled;

  config.eyeBrightness = oldConfig.eyeBrightness;
  config.ledEffectSpeed = oldConfig.ledEffectSpeed;
  config.eyeVersion = oldConfig.eyeVersion;
  config.statusLedBrightness = oldConfig.statusLedBrightness;
  config.statusLedEnabled = oldConfig.statusLedEnabled;

  config.scanEyeMoveMin = oldConfig.scanEyeMoveMin;
  config.scanEyeMoveMax = oldConfig.scanEyeMoveMax;
  config.scanEyeWaitMin = oldConfig.scanEyeWaitMin;
  config.scanEyeWaitMax = oldConfig.scanEyeWaitMax;
  config.alertEyeMoveMin = oldConfig.alertEyeMoveMin;
  config.alertEyeMoveMax = oldConfig.alertEyeMoveMax;
  config.alertEyeWaitMin = oldConfig.alertEyeWaitMin;
  config.alertEyeWaitMax = oldConfig.alertEyeWaitMax;
  config.soundPauseMin = oldConfig.soundPauseMin;
  config.soundPauseMax = oldConfig.soundPauseMax;
  config.bootSequenceDelay = oldConfig.bootSequenceDelay;

  memcpy(config.profiles, oldConfig.profiles, sizeof(config.profiles));

  for (int i = 0; i < 21; i++) {
    memcpy(config.buttons[i].name, oldConfig.buttons[i].name, sizeof(config.buttons[i].name));
    config.buttons[i].code = oldConfig.buttons[i].code;
    config.buttons[i].isConfigured = oldConfig.buttons[i].isConfigured;
    memset(config.buttons[i].sequenceName, 0, sizeof(config.buttons[i].sequenceName));
  }
...
static String buildApiJson(bool ok, const String& message, const String& extraJson = "") {
  String json = "{\"ok\":";
  json += ok ? "true" : "false";
  json += ",\"message\":\"";
  json += escapeJsonString(message.c_str());
  json += "\"";
...
static void sendApiResponse(int statusCode, bool ok, const String& message, const String& extraJson = "") {
  server.send(statusCode, "application/json", buildApiJson(ok, message, extraJson));
}
...
void handleSetServos() {
  ...
  if (server.hasArg("eyePan") && server.hasArg("eyeTilt") &&
    ...
    int eyePanPos, eyeTiltPos, headPanPos, headTiltPos;

    // Safe parsing with validation
    if (!safeParseInt(server.arg("eyePan"), eyePanPos, 0, 180) ||
        !safeParseInt(server.arg("eyeTilt"), eyeTiltPos, 0, 180) ||
        !safeParseInt(server.arg("headPan"), headPanPos, 0, 180) ||
        !safeParseInt(server.arg("headTilt"), headTiltPos, 0, 180)) {
      server.send(400, "text/plain", "Invalid servo position values (must be 0-180)");
      return;
    }

    // Validate positions within configured limits
    ...
    // Update current positions and mark movement complete
    ...
    // Mark servos as not moving since we wrote directly
...
void handleBrightness() {
  ...
  if (server.hasArg("value")) {
    int brightness;
    if (!safeParseInt(server.arg("value"), brightness, 0, 255)) {
...
void handleVolume() {
  ...
  if (server.hasArg("value")) {
    int volume;
    if (!safeParseInt(server.arg("value"), volume, 0, 30)) {
      server.send(400, "text/plain", "Invalid volume value (must be 0-30)");
      return;
    }
    setVolume(volume);
...
void handlePlaySound() {
  ...
  if (server.hasArg("file")) {
    int fileNum;
    if (!safeParseInt(server.arg("file"), fileNum, 1, 255)) {
      server.send(400, "text/plain", "Invalid file number (must be 1-255)");
      return;
    }
    playSound(fileNum);
...
void handleDetailCount() {
  ...
  if (server.hasArg("value")) {
    int count;
    if (!safeParseInt(server.arg("value"), count, 1, 8)) {
      server.send(400, "text/plain", "Invalid count value (must be 1-8)");
      return;
    }
...
void processCommand(String fullCommand) {
  ...
  switch (command) {
    ...
    case CMD_CLEAR: {
      Serial.print("Clear all data? Type 'YES' to confirm (30s timeout): ");
      if (!waitForSerialWithTimeout(30000)) {
...
void enterLearningMode() {
  ...
  if (config.buttonCount == 0) {
    Serial.print("How many buttons does your remote have? (1-21, 30s timeout): ");
    String buttonInput;
    unsigned long inputTimeout = millis();
    while (Serial.available() == 0) {
      delay(50);
      if (millis() - inputTimeout > 30000) {
        Serial.println(F("\n⏱️ Timeout (30s) - using default 17 buttons."));
...
void handleIRCommand(uint32_t code) {
  ...
  for (int i = 0; i < config.buttonCount; i++) {
    if (config.buttons[i].isConfigured && config.buttons[i].code == code) {
      ...
      // Check if a sequence is mapped to this button
      if (strlen(config.buttons[i].sequenceName) > 0) {
        Serial.print(F("▶️ IR triggering sequence: "));
        Serial.println(config.buttons[i].sequenceName);
        sequenceManager.playSequence(config.buttons[i].sequenceName, false);
        return;
      }

      // Otherwise execute standard button command
...
void handleServoCommand(String params) {
  ...
  if (args[0] == "show") {
    ...
    int panMin = constrain(args[2].toInt(), 0, 180);
    int panMax = constrain(args[3].toInt(), 0, 180);
    int tiltMin = constrain(args[4].toInt(), 0, 180);
    int tiltMax = constrain(args[5].toInt(), 0, 180);

    // Ensure min < max
    if (panMin >= panMax || tiltMin >= tiltMax) {
      Serial.println(F("❌ Error: Minimum must be less than maximum"));
      return;
    }

    config.eyePanMin = panMin;
    config.eyePanMax = panMax;
    config.eyeTiltMin = tiltMin;
    config.eyeTiltMax = tiltMax;
    ...
    config.headPanMin = panMin;
    config.headPanMax = panMax;
    config.headTiltMin = tiltMin;
    config.headTiltMax = tiltMax;
...
void handleWiFiCommand(String params) {
  ...
  if (subCmd == "show") {
    ...
    Serial.print(F("Clear WiFi configuration? Type 'YES' to confirm (30s timeout): "));
    if (!waitForSerialWithTimeout(30000)) {
      Serial.println(F("Timeout - operation cancelled."));
      return;
    }
    String confirmation = Serial.readStringUntil('\n');
    confirmation.trim();
    ...
    // Stop web server first and allow pending requests to complete
    server.stop();
    Serial.println(F("Web server stopped"));
    delay(500);  // Allow any in-flight requests to complete

    // Disconnect WiFi and wait for proper cleanup
    WiFi.disconnect(true);  // true = wifioff
    Serial.println(F("WiFi disconnected, waiting for cleanup..."));
    delay(1500);  // Increased delay to allow proper cleanup and socket release

    // Stop MDNS service
    MDNS.end();
    delay(200);

    // Clear any WiFi events that may have queued
    ...
    initializeWiFi();
    delay(100);  // Brief pause before starting web server
    setupWebServer();
...
void handleAPCommand(String params) {
  ...
  if (subCmd == "show") {
    ...
    if (config.apConfigured && strlen(config.apSSID) > 0) {
      ...
      Serial.print(F("AP Password: "));
      Serial.print(DEFAULT_AP_PASSWORD);
    ...
    Serial.print(F("Reset AP configuration to defaults? Type 'YES' to confirm (30s timeout): "));
    if (!waitForSerialWithTimeout(30000)) {
      Serial.println(F("Timeout - operation cancelled."));
      return;
    }
    ...
    if (confirmation == "YES") {
      ...
      Serial.print(F("Default AP will be K2SO-XXXXXX with password: "));
...
void handleProfileCommand(String params) {
  ...
  if (args[0] == "list") {
    ...
    if (slot < 0) {
      Serial.println(F("❌ All 5 profile slots are full!"));
      Serial.println(F("   Delete a profile first with: profile delete <slot>"));
      Serial.println(F("   Use 'profile list' to see all profiles"));
      return;
    }
...
void showHelp() {
  ...
  Serial.println("\nSEQUENCE RECORDING:");
  Serial.println(F("  seq [options]   - Record and play animation sequences"));
  Serial.println(F("                    Type 'seq' for detailed commands"));
...
void clearAllData() {
  ...
  Serial.print("Are you absolutely sure? Type 'YES' to confirm (30s timeout): ");
  if (!waitForSerialWithTimeout(30000)) {
    Serial.println(F("Timeout - operation cancelled."));
    return;
  }
...
void handleBootSequence(unsigned long currentMillis) {
  ...
  static bool use7LEDSequence = false;

  // Initialize on first run to avoid huge time difference
  if (firstRun) {
    lastBootStep = currentMillis;
    firstRun = false;
    // Determine if we should use 7-LED simplified sequence
    use7LEDSequence = (activeEyeLEDCount != 13);
    if (use7LEDSequence) {
      Serial.println(F("Boot: Using simplified sequence for 7-LED eyes"));
    }
  }
  ...
  if (currentMillis - lastBootStep >= config.bootSequenceDelay) {
    ...
    // Simplified boot sequence for 7-LED eyes
    if (use7LEDSequence) {
      switch(bootSequenceStep) {
        case 0:
          Serial.println(F("Boot: Initializing eye awakening (7-LED mode)..."));
          leftEye.clear();
          rightEye.clear();
          leftEye.show();
          rightEye.show();
        ...
        case 1: {
          // Weak pulse
          uint32_t weakPulse = Adafruit_NeoPixel::Color(20, 30, 35);
          for (int i = 0; i < activeEyeLEDCount; i++) {
            leftEye.setPixelColor(i, weakPulse);
            rightEye.setPixelColor(i, weakPulse);
          }
          leftEye.show();
          rightEye.show();
          bootSequenceStep++;
        ...
        case 3: {
          // Stronger pulse
          uint32_t strongerPulse = Adafruit_NeoPixel::Color(50, 70, 85);
          for (int i = 0; i < activeEyeLEDCount; i++) {
            leftEye.setPixelColor(i, strongerPulse);
            rightEye.setPixelColor(i, strongerPulse);
          }
          leftEye.show();
          rightEye.show();
          bootSequenceStep++;
        ...
        case 4: {
          // 75% power
          uint32_t bright = Adafruit_NeoPixel::Color(100, 140, 170);
          for (int i = 0; i < activeEyeLEDCount; i++) {
            leftEye.setPixelColor(i, bright);
            rightEye.setPixelColor(i, bright);
          }
          leftEye.show();
          rightEye.show();
          bootSequenceStep++;
        ...
        case 5:
          ...
          setEyeColor(getIceBlue(), getIceBlue());
          bootSequenceStep = 25;  // Skip to audio step
          break;
        default:
          // Fall through to common completion steps (25+)
          break;
      }
      // If we've handled a 7-LED step, check if we need to continue to common steps
      if (bootSequenceStep >= 25) {
        use7LEDSequence = false;  // Use common steps from here
      } else {
        return;  // Return early for 7-LED specific steps
      }
    }

    // Full 13-LED boot sequence
    switch(bootSequenceStep) {
...
void loadConfiguration() {
  ...
  if (config.magic != (uint8_t)EEPROM_MAGIC) {
    Serial.println("Initializing configuration with defaults...");
    setDefaultConfiguration();
    needsDefaults = true;
    saveConfiguration();
  } else if (config.version == 1) {
    ConfigDataV1 oldConfig;
    EEPROM.get(0, oldConfig);

    uint32_t storedLegacyChecksum = oldConfig.checksum;
    oldConfig.checksum = 0;
    uint32_t calculatedLegacyChecksum = calculateChecksumForConfigV1(oldConfig);
    oldConfig.checksum = storedLegacyChecksum;

    if (storedLegacyChecksum != calculatedLegacyChecksum) {
      Serial.println("Legacy configuration checksum mismatch, reloading defaults");
      setDefaultConfiguration();
      needsDefaults = true;
      saveConfiguration();
    } else {
      applyLegacyConfigV1ToCurrent(oldConfig);
      saveConfiguration();
      Serial.println("Config migrated from v1 (v1.2.3/v1.2.4) to v2 (v1.3.0). All settings preserved.");
    }
  } else if (config.version != 2) {
    Serial.println("Unsupported configuration version, reloading defaults");
    setDefaultConfiguration();
    needsDefaults = true;
    saveConfiguration();
  }
...
uint32_t calculateChecksum() {
  return calculateChecksumForConfig(config);
}
...
void restoreFromSerial() {
  ...
  Serial.println(F("WARNING:"));
  Serial.print("Continue? Type 'YES' to proceed (30s timeout): ");
  if (!waitForSerialWithTimeout(30000)) {
    Serial.println(F("Timeout - operation cancelled."));
    return;
  }
  ...
  const size_t currentHexLength = sizeof(ConfigData) * 2;
  const size_t legacyHexLength = sizeof(ConfigDataV1) * 2;

  // Non-blocking multi-line data collection with timeout
  String hexData = "";
  hexData.reserve(currentHexLength);
  unsigned long dataTimeout = millis();
  unsigned long lastInputAt = millis();
  const unsigned long DATA_TIMEOUT_MS = 60000;
  const unsigned long QUIET_AFTER_DATA_MS = 250;

  while (millis() - dataTimeout < DATA_TIMEOUT_MS) {
    if (Serial.available()) {
      char c = Serial.read();
      if (isxdigit(c)) {
        hexData += c;
        ...
        Serial.printf("Error: Invalid character '%c' in restore data.\n", c);
        return;
      }
    }

    if (hexData.length() >= currentHexLength) {
      break;
    }

    if (hexData.length() >= legacyHexLength &&
        millis() - lastInputAt > QUIET_AFTER_DATA_MS) {
      break;
    }
  ...
  if (hexData.length() == 0) {
    Serial.println(F("⏱️ Timeout (60s) - no data received. Operation cancelled."));
    return;
  }

  if (hexData.length() < legacyHexLength) {
    Serial.printf("Error: Data too short. Expected %d characters, got %d\n",
                  legacyHexLength, hexData.length());
    return;
  }
  ...
  if (hexData.length() >= currentHexLength) {
    ...
    uint8_t* data = reinterpret_cast<uint8_t*>(&tempConfig);
    ...
    if (tempConfig.magic == (uint8_t)EEPROM_MAGIC && tempConfig.version == 2) {
      uint32_t incomingChecksum = tempConfig.checksum;
      tempConfig.checksum = 0;
      uint32_t calculatedChecksum = calculateChecksumForConfig(tempConfig);
      tempConfig.checksum = incomingChecksum;
      if (incomingChecksum == calculatedChecksum) {
        memcpy(&config, &tempConfig, sizeof(config));
        saveConfiguration();
        applyConfiguration();
        restored = true;
        restoreMessage = "Configuration v2 restored successfully.";
      }
    }
  }

  if (!restored && hexData.length() >= legacyHexLength) {
    ConfigDataV1 legacyConfig;
    uint8_t* data = reinterpret_cast<uint8_t*>(&legacyConfig);

    for (size_t i = 0; i < sizeof(ConfigDataV1); i++) {
      String hexByte = hexData.substring(i * 2, i * 2 + 2);
      data[i] = strtol(hexByte.c_str(), NULL, 16);
    }

    if (legacyConfig.magic == (uint8_t)EEPROM_MAGIC && legacyConfig.version == 1) {
      uint32_t incomingChecksum = legacyConfig.checksum;
      legacyConfig.checksum = 0;
      uint32_t calculatedChecksum = calculateChecksumForConfigV1(legacyConfig);
      legacyConfig.checksum = incomingChecksum;
      if (incomingChecksum == calculatedChecksum) {
        applyLegacyConfigV1ToCurrent(legacyConfig);
        saveConfiguration();
        applyConfiguration();
        restored = true;
        restoreMessage = "Legacy configuration restored and migrated to v2 successfully.";
      }
    }
  }
  ...
  if (!restored) {
    ...
    Serial.printf("  Received %d hex chars\n", hexData.length());
    Serial.printf("  Expected %d (v1) or %d (v2)\n", legacyHexLength, currentHexLength);
    return;
  }

  Serial.println(restoreMessage);
  Serial.println(F("System will restart in 2 seconds..."));
  delay(2000);
  ESP.restart();
}
//========================================
// SEQUENCE RECORDING/PLAYBACK FUNCTIONS
//========================================

void handleSequenceCommand(String params) {
  params.trim();

  if (params.length() == 0) {
    Serial.println(F("\n┌─ Sequence Commands ────────────────────────────"));
    Serial.println(F("│ RECORDING:"));
    Serial.println(F("│  seq new <name>           - Start new recording"));
    Serial.println(F("│  seq frame <duration_ms>  - Add current state as frame"));
    Serial.println(F("│  seq save                 - Save recording"));
    Serial.println(F("│  seq cancel               - Cancel recording"));
    Serial.println(F("│  seq export <name>        - Export raw sequence JSON"));
    Serial.println(F("│  seq import               - Import raw sequence JSON"));
    Serial.println(F("│"));
    Serial.println(F("│ PLAYBACK:"));
    Serial.println(F("│  seq play <name>          - Play sequence"));
    Serial.println(F("│  seq loop <name>          - Play sequence (looped)"));
    Serial.println(F("│  seq stop                 - Stop playback"));
    Serial.println(F("│"));
    Serial.println(F("│ MANAGEMENT:"));
    Serial.println(F("│  seq list                 - List all sequences"));
    Serial.println(F("│  seq info <name>          - Show sequence details"));
    Serial.println(F("│  seq verify <name>        - Verify sequence file integrity"));
    Serial.println(F("│  seq delete <name>        - Delete sequence"));
    Serial.println(F("│  seq rename <old> <new>   - Rename sequence"));
    Serial.println(F("│"));
    Serial.println(F("│ PLAYLIST (CHAINING):"));
    Serial.println(F("│  seq playlist             - Show playlist"));
    Serial.println(F("│  seq playlist add <name>  - Add sequence to playlist"));
    Serial.println(F("│  seq playlist remove <n>  - Remove playlist item"));
    Serial.println(F("│  seq playlist move <a> <b>- Move playlist item"));
    Serial.println(F("│  seq playlist clear       - Clear playlist"));
    Serial.println(F("│  seq playlist play        - Play playlist"));
    Serial.println(F("│  seq playlist loop        - Loop playlist"));
    Serial.println(F("│"));
    Serial.println(F("│ IR MAPPING:"));
    Serial.println(F("│  seq map                  - Show IR button mappings"));
    Serial.println(F("│  seq map <button> <name>  - Map IR button to sequence"));
    Serial.println(F("│  seq map <button> clear   - Clear button mapping"));
    Serial.println(F("│"));
    Serial.println(F("│ STATUS:"));
    Serial.println(F("│  seq status               - Show current status"));
    Serial.println(F("└────────────────────────────────────────────────"));
    return;
  }
  ...
  if (subCmd == "new") {
    if (subParams.length() == 0) {
      Serial.println(F("❌ Usage: seq new <name>"));
      return;
    }
    if (!sequenceManager.isValidSequenceName(subParams.c_str())) {
      Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
      return;
    }
    if (sequenceManager.startRecording(subParams.c_str())) {
      Serial.println(F("✓ Recording started. Use 'seq frame <duration>' to add frames."));
    }
    return;
  }

  if (subCmd == "frame") {
    if (!sequenceManager.isRecording()) {
      Serial.println(F("❌ Not recording. Start with 'seq new <name>' first."));
      return;
    }

    uint16_t duration = 1000; // Default 1 second
    if (subParams.length() > 0) {
      duration = subParams.toInt();
      if (duration < 1 || duration > 60000) {
        Serial.println(F("❌ Duration must be 1-60000 ms"));
        return;
      }
    }

    if (sequenceManager.addCurrentStateAsFrame(duration)) {
      Serial.print(F("✓ Frame "));
      Serial.print(sequenceManager.getRecordingFrameCount());
      Serial.print(F(" added ("));
      Serial.print(duration);
      Serial.println(F("ms)"));

      // Show what was captured
      Serial.print(F("  Servos: EP="));
      Serial.print(eyePan.currentPosition);
      Serial.print(F(" ET="));
      Serial.print(eyeTilt.currentPosition);
      Serial.print(F(" HP="));
      Serial.print(headPan.currentPosition);
      Serial.print(F(" HT="));
      Serial.println(headTilt.currentPosition);
    }
    return;
  }
  ...
  if (subCmd == "save") {
    ...
    sequenceManager.saveRecording();
    return;
  }

  if (subCmd == "export") {
    if (subParams.length() == 0) {
      Serial.println(F("❌ Usage: seq export <name>"));
      return;
    }
    if (!sequenceManager.isValidSequenceName(subParams.c_str())) {
      Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
    ...
    String json;
    String errorMessage;
    if (!sequenceManager.exportSequenceJson(subParams.c_str(), json, errorMessage)) {
      Serial.print(F("❌ Export failed: "));
      Serial.println(errorMessage);
      return;
    }

    Serial.println(F("=== SEQUENCE EXPORT START ==="));
    Serial.println(json);
    Serial.println(F("=== SEQUENCE EXPORT END ==="));
    return;
  }

  if (subCmd == "import") {
    if (subParams.length() > 0) {
      Serial.println(F("❌ Usage: seq import"));
      return;
    }

    Serial.println(F("Paste raw sequence JSON. Import starts automatically after a short quiet period."));
    String importJson = "";
    importJson.reserve(16384);
    unsigned long inputTimeout = millis();
    unsigned long lastInputAt = millis();
    const unsigned long INPUT_TIMEOUT_MS = 60000;
    const unsigned long QUIET_AFTER_DATA_MS = 250;

    while (millis() - inputTimeout < INPUT_TIMEOUT_MS) {
      if (Serial.available()) {
        char c = Serial.read();
        importJson += c;
        inputTimeout = millis();
      ...
      if (importJson.length() > 0 && millis() - lastInputAt > QUIET_AFTER_DATA_MS) {
        break;
      }
    ...
    if (importJson.length() == 0) {
      Serial.println(F("❌ No JSON received for import."));
      return;
    }

    String importedName;
    String errorMessage;
    if (!sequenceManager.importSequenceJson(importJson, importedName, errorMessage)) {
      Serial.print(F("❌ Import failed: "));
      Serial.println(errorMessage);
      return;
    }

    Serial.print(F("✓ Imported sequence: "));
    Serial.println(importedName);
    return;
  }

  if (subCmd == "format") {
    Serial.println(F("WARNING: seq format will erase all saved sequences."));
    Serial.print(F("Type 'YES' to confirm (30s timeout): "));
    if (!waitForSerialWithTimeout(30000)) {
      Serial.println(F("Timeout - format cancelled."));
      return;
    }
    String confirm = Serial.readStringUntil('\n');
    confirm.trim();
    ...
    if (sequenceManager.formatStorage()) {
      Serial.println(F("Sequence storage formatted."));
    } else {
      Serial.println(F("Failed to format sequence storage."));
    }
    return;
  }

  if (subCmd == "cancel") {
    if (sequenceManager.cancelRecording()) {
      Serial.println(F("✓ Recording cancelled"));
    } else {
      Serial.println(F("❌ Not recording"));
    }
    return;
  ...
  if (subCmd == "play") {
    if (subParams.length() == 0) {
      Serial.println(F("❌ Usage: seq play <name>"));
      return;
    }
    if (!sequenceManager.isValidSequenceName(subParams.c_str())) {
      Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
    ...
    sequenceManager.playSequence(subParams.c_str(), false);
    return;
  }

  if (subCmd == "loop") {
    if (subParams.length() == 0) {
      Serial.println(F("❌ Usage: seq loop <name>"));
      return;
    }
    if (!sequenceManager.isValidSequenceName(subParams.c_str())) {
      Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
    ...
    sequenceManager.playSequence(subParams.c_str(), true);
    return;
  }
  ...
  if (subCmd == "stats") {
    SequenceStorageStats stats;
    if (!sequenceManager.getStorageStats(stats)) {
      Serial.println(F("Failed to read sequence storage stats."));
      return;
    }

    Serial.println(F("\nSequence Storage Stats"));
    Serial.print(F("  Sequences: "));
    Serial.println(stats.sequenceCount);
    Serial.print(F("  Saved playlists: "));
    Serial.println(stats.playlistCount);
    Serial.print(F("  Total bytes: "));
    Serial.println((unsigned long)stats.totalBytes);
    Serial.print(F("  Used bytes: "));
    Serial.println((unsigned long)stats.usedBytes);
    Serial.print(F("  Free bytes: "));
    Serial.println((unsigned long)stats.freeBytes);
    Serial.print(F("  Largest sequence: "));
    if (stats.largestSequenceBytes > 0) {
      Serial.print(stats.largestSequenceName);
      Serial.print(F(" ("));
      Serial.print((unsigned long)stats.largestSequenceBytes);
  ...
  if (subCmd == "info") {
    if (subParams.length() == 0) {
      Serial.println(F("❌ Usage: seq info <name>"));
      return;
    }
    if (!sequenceManager.isValidSequenceName(subParams.c_str())) {
      Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
    ...
    sequenceManager.printSequenceInfo(subParams.c_str());
    return;
  }

  if (subCmd == "verify" && subParams.equalsIgnoreCase("all")) {
    uint16_t okCount = 0;
    uint16_t failCount = 0;
    String report;
    bool allOk = sequenceManager.verifyAllSequences(okCount, failCount, report);
    Serial.println(F("\nSequence Verify All"));
    Serial.print(F("  OK: "));
    ...
    if (!allOk) {
      Serial.println(F("One or more sequences failed verification."));
    }
    return;
  }
  ...
  if (subCmd == "verify") {
    if (subParams.length() == 0) {
      Serial.println(F("❌ Usage: seq verify <name>"));
      return;
    }
    if (!sequenceManager.isValidSequenceName(subParams.c_str())) {
      Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
    ...
    if (subParams.equalsIgnoreCase("all")) {
      uint16_t okCount = 0;
      uint16_t failCount = 0;
      String report;
      bool allOk = sequenceManager.verifyAllSequences(okCount, failCount, report);
    ...
    SequenceVerifyInfo verifyInfo;
    String errorMessage;
    bool ok = sequenceManager.verifySequence(subParams.c_str(), verifyInfo, errorMessage);

    Serial.println(F("\nSequence Verify"));
    Serial.print(F("  Name: "));
    Serial.println(subParams);
    Serial.print(F("  File size: "));
    Serial.print(verifyInfo.fileSize);
    Serial.println(F(" bytes"));
    Serial.print(F("  Declared frames: "));
    Serial.println(verifyInfo.declaredFrameCount);
    Serial.print(F("  Actual frames: "));
    Serial.println(verifyInfo.actualFrameCount);
    Serial.print(F("  Duration: "));
    Serial.print(verifyInfo.totalDuration / 1000.0, 2);
    Serial.println(F(" seconds"));
    Serial.print(F("  Version: "));
    Serial.println(verifyInfo.version);
  ...
  if (subCmd == "duplicate") {
    int space = subParams.indexOf(' ');
    if (space < 0) {
      Serial.println(F("Usage: seq duplicate <old_name> <new_name>"));
      return;
    }

    String oldName = subParams.substring(0, space);
    String newName = subParams.substring(space + 1);
    oldName.trim();
    newName.trim();

    if (!sequenceManager.isValidSequenceName(oldName.c_str()) ||
        !sequenceManager.isValidSequenceName(newName.c_str())) {
      Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
      return;
    }

    if (!sequenceManager.duplicateSequence(oldName.c_str(), newName.c_str())) {
      Serial.println(F("Sequence duplicate failed."));
    }
    return;
  }
  ...
  if (subCmd == "rename") {
    int space = subParams.indexOf(' ');
    if (space < 0) {
      Serial.println(F("❌ Usage: seq rename <old_name> <new_name>"));
      return;
    }
    String oldName = subParams.substring(0, space);
    String newName = subParams.substring(space + 1);
    ...
    if (sequenceManager.renameSequence(oldName.c_str(), newName.c_str())) {
      Serial.print(F("✓ Renamed '"));
  ...
  // ========== PLAYLIST COMMANDS (CHAINING) ==========

  if (subCmd == "playlist") {
    if (subParams.length() == 0) {
      // Show current playlist
      sequenceManager.playlistPrint();
      return;
    }

    // Parse playlist subcommand
    int space = subParams.indexOf(' ');
    String plCmd = (space > 0) ? subParams.substring(0, space) : subParams;
    String plParams = (space > 0) ? subParams.substring(space + 1) : "";
    plCmd.toLowerCase();
    plParams.trim();

    if (plCmd == "add") {
      if (plParams.length() == 0) {
        Serial.println(F("❌ Usage: seq playlist add <sequence_name>"));
        return;
      }
      if (!sequenceManager.isValidSequenceName(plParams.c_str())) {
        Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
        return;
      }
      sequenceManager.playlistAdd(plParams.c_str());
      return;
    }

    if (plCmd == "list") {
      char playlistNames[20][MAX_SEQUENCE_NAME_LENGTH];
      int playlistCount = sequenceManager.listPlaylists(playlistNames, 20);
      if (playlistCount == 0) {
        Serial.println(F("No saved playlists found."));
        return;
      }
    ...
    if (plCmd == "remove") {
      int index = 0;
      if (!safeParseInt(plParams, index, 1, MAX_PLAYLIST_ITEMS)) {
        Serial.println(F("❌ Usage: seq playlist remove <item_number>"));
        return;
      }
      sequenceManager.playlistRemove((uint8_t)(index - 1));
      return;
    }

    if (plCmd == "move") {
      int moveSpace = plParams.indexOf(' ');
      if (moveSpace < 0) {
        Serial.println(F("❌ Usage: seq playlist move <from> <to>"));
        return;
      }

      String fromArg = plParams.substring(0, moveSpace);
      String toArg = plParams.substring(moveSpace + 1);
      fromArg.trim();
      toArg.trim();

      int fromIndex = 0;
      int toIndex = 0;
      if (!safeParseInt(fromArg, fromIndex, 1, MAX_PLAYLIST_ITEMS) ||
          !safeParseInt(toArg, toIndex, 1, MAX_PLAYLIST_ITEMS)) {
        Serial.println(F("❌ Usage: seq playlist move <from> <to>"));
        return;
      }

      sequenceManager.playlistMove((uint8_t)(fromIndex - 1), (uint8_t)(toIndex - 1));
      return;
    }

    if (plCmd == "save") {
      if (plParams.length() == 0) {
        Serial.println(F("Usage: seq playlist save <name>"));
        return;
      }
      if (!sequenceManager.isValidSequenceName(plParams.c_str())) {
        Serial.println(F("Invalid playlist name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
        return;
      }
      if (!sequenceManager.playlistSave(plParams.c_str())) {
        Serial.println(F("Playlist save failed."));
      }
      return;
    }
    ...
    if (plCmd == "load") {
      if (plParams.length() == 0) {
        Serial.println(F("Usage: seq playlist load <name>"));
        return;
      }
      if (!sequenceManager.isValidSequenceName(plParams.c_str())) {
        Serial.println(F("Invalid playlist name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
      ...
      if (!sequenceManager.playlistLoad(plParams.c_str())) {
        Serial.println(F("Playlist load failed."));
      }
      return;
    }

    if (plCmd == "clear") {
      sequenceManager.playlistClear();
      return;
    }

    if (plCmd == "play") {
      sequenceManager.playlistStart(false);
      return;
    }

    if (plCmd == "loop") {
      sequenceManager.playlistStart(true);
      return;
    }

    Serial.println(F("❌ Unknown playlist command"));
    Serial.println(F("  playlist        - Show playlist"));
    Serial.println(F("  playlist add <name> - Add sequence"));
    Serial.println(F("  playlist list - List saved playlists"));
    Serial.println(F("  playlist remove <n> - Remove item"));
    Serial.println(F("  playlist move <a> <b> - Reorder items"));
    Serial.println(F("  playlist save <name> - Save playlist"));
    Serial.println(F("  playlist load <name> - Load playlist"));
    Serial.println(F("  playlist clear  - Clear playlist"));
    Serial.println(F("  playlist play   - Play playlist"));
    Serial.println(F("  playlist loop   - Loop playlist"));
    return;
  }
  ...
  if (subCmd == "map") {
    if (subParams.length() == 0) {
      // Show all IR mappings
      Serial.println(F("\n┌─ IR Button Mappings ───────────────────────────"));
      bool hasMappings = false;
      for (int i = 0; i < config.buttonCount; i++) {
        if (config.buttons[i].isConfigured && strlen(config.buttons[i].sequenceName) > 0) {
          Serial.print(F("│ "));
          Serial.print(config.buttons[i].name);
          Serial.print(F(" → "));
          Serial.println(config.buttons[i].sequenceName);
          hasMappings = true;
        }
      ...
      if (!hasMappings) {
        Serial.println(F("│ No IR mappings configured"));
      }
      Serial.println(F("└────────────────────────────────────────────────"));
      return;
    }

    // Parse: map <button_name> <sequence_name>
    int space = subParams.indexOf(' ');
    if (space < 0) {
      Serial.println(F("❌ Usage: seq map <button_name> <sequence_name>"));
      Serial.println(F("   Or: seq map <button_name> clear"));
      return;
    }

    String buttonName = subParams.substring(0, space);
    String seqName = subParams.substring(space + 1);
    buttonName.trim();
    seqName.trim();
    ...
    int buttonIndex = -1;
    for (int i = 0; i < config.buttonCount; i++) {
      if (config.buttons[i].isConfigured && strcmp(config.buttons[i].name, buttonName.c_str()) == 0) {
        buttonIndex = i;
        break;
      }
    }

    if (buttonIndex < 0 || buttonIndex >= 21 || buttonIndex >= config.buttonCount) {
      Serial.print(F("❌ Button not found or invalid: "));
      Serial.println(buttonName);
      Serial.println(F("   Use 'show' command to see configured buttons"));
      return;
    }

    // Clear mapping
    if (seqName.equalsIgnoreCase("clear")) {
      config.buttons[buttonIndex].sequenceName[0] = '\0';
      smartSaveToEEPROM();
      Serial.print(F("✓ Cleared mapping for button: "));
      Serial.println(buttonName);
      return;
    }

    if (!sequenceManager.isValidSequenceName(seqName.c_str())) {
      Serial.println(F("Invalid sequence name. Use only A-Z, a-z, 0-9, _ and - (1-31 chars)."));
      return;
    }

    // Check if sequence exists
    if (!sequenceManager.sequenceExists(seqName.c_str())) {
    ...
    // Set mapping
    strncpy(config.buttons[buttonIndex].sequenceName, seqName.c_str(), 31);
    config.buttons[buttonIndex].sequenceName[31] = '\0';
    smartSaveToEEPROM();

    Serial.print(F("✓ Mapped button '"));
    Serial.print(buttonName);
    Serial.print(F("' → sequence '"));
    Serial.print(seqName);
    Serial.println(F("'"));
    return;
  }
  ...
  if (subCmd == "status") {
    ...
    Serial.println(sequenceManager.isStorageAvailable() ? F("LittleFS OK") : F("Not available"));
    Serial.print(F("│ State: "));
    Serial.println(sequenceManager.getStatusString());

    if (sequenceManager.isRecording()) {
      Serial.print(F("│ Recording: "));
      Serial.println(sequenceManager.getRecordingName());
      Serial.print(F("│ Frames: "));
      Serial.println(sequenceManager.getRecordingFrameCount());
    }

    if (sequenceManager.isPlaying()) {
      Serial.print(F("│ Progress: "));
      Serial.print(sequenceManager.getCurrentFrame() + 1);
      Serial.print(F(" / "));
      Serial.print(sequenceManager.getTotalFrames());
      Serial.print(F(" ("));
      Serial.print((int)(sequenceManager.getPlaybackProgress() * 100));
      Serial.println(F("%)"));
    }
  ...
  Serial.println(F("❌ Unknown seq command. Type 'seq' for help."));
}

//========================================
// SEQUENCE WEB HANDLERS
//========================================
...
void handleSeqList() {
  ...
  String json = "{\"ok\":true,\"message\":\"Sequence list loaded.\",\"sequences\":[";
  for (int i = 0; i < count; i++) {
    if (i > 0) json += ",";
    json += "{\"name\":\"";
...
void handleSeqPlay() {
  ...
  if (!server.hasArg("name")) {
    sendApiResponse(400, false, "Missing name parameter.");
    return;
  }

  String name = server.arg("name");
  if (!sequenceManager.isValidSequenceName(name.c_str())) {
    sendApiResponse(400, false, "Invalid sequence name.");
    return;
  }
  if (!sequenceManager.sequenceExists(name.c_str())) {
    sendApiResponse(404, false, "Sequence not found.");
    return;
  }

  bool success = sequenceManager.playSequence(name.c_str(), false);
  sendApiResponse(success ? 200 : 500,
                  success,
                  success ? String("Playing sequence: ") + name : String("Failed to play sequence: ") + name);
}

void handleSeqStop() {
  if (!checkWebAuth()) return;

  bool stopped = sequenceManager.stopPlayback();
  sendApiResponse(200, true, stopped ? "Playback stopped." : "No playback was active.");
}
...
void handleSeqPause() {
  ...
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? "Playback paused." : "No active playback to pause.");
}
...
void handleSeqResume() {
  ...
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? "Playback resumed." : "No paused playback to resume.");
}
...
void handleSeqLoop() {
  ...
  sendApiResponse(success ? 200 : 500,
                  success,
                  success ? String("Looping sequence: ") + name : String("Failed to loop sequence: ") + name);
}
...
void handleSeqDelete() {
  ...
  bool success = sequenceManager.deleteSequence(name.c_str());
  sendApiResponse(success ? 200 : 500,
                  success,
                  success ? String("Deleted sequence: ") + name : String("Failed to delete sequence: ") + name);
}

void handleSeqVerify() {
  if (!checkWebAuth()) return;
  ...
  SequenceVerifyInfo info;
  String errorMessage;
  bool ok = sequenceManager.verifySequence(name.c_str(), info, errorMessage);

  String extraJson = "\"name\":\"" + escapeJsonString(name.c_str()) + "\",";
  extraJson += "\"declaredFrames\":" + String(info.declaredFrameCount) + ",";
  extraJson += "\"actualFrames\":" + String(info.actualFrameCount) + ",";
  extraJson += "\"durationMs\":" + String(info.totalDuration) + ",";
  extraJson += "\"version\":" + String(info.version) + ",";
  extraJson += "\"fileSize\":" + String((unsigned long)info.fileSize);

  sendApiResponse(ok ? 200 : 422, ok, errorMessage, extraJson);
}

void handleSeqVerifyAll() {
  if (!checkWebAuth()) return;
  ...
  String extraJson = "\"okCount\":" + String(okCount) + ",";
  extraJson += "\"failCount\":" + String(failCount) + ",";
  extraJson += "\"report\":\"" + escapeJsonString(report.c_str()) + "\"";
  sendApiResponse(allOk ? 200 : 422,
                  allOk,
                  allOk ? "All sequences verified." : "One or more sequences failed verification.",
                  extraJson);
}
...
void handleSeqStats() {
  ...
  SequenceStorageStats stats;
  if (!sequenceManager.getStorageStats(stats)) {
    sendApiResponse(500, false, "Failed to read sequence storage stats.");
    return;
  }

  String extraJson = "\"sequenceCount\":" + String(stats.sequenceCount) + ",";
  extraJson += "\"playlistCount\":" + String(stats.playlistCount) + ",";
  extraJson += "\"totalBytes\":" + String((unsigned long)stats.totalBytes) + ",";
  extraJson += "\"usedBytes\":" + String((unsigned long)stats.usedBytes) + ",";
  extraJson += "\"freeBytes\":" + String((unsigned long)stats.freeBytes) + ",";
  extraJson += "\"largestSequenceBytes\":" + String((unsigned long)stats.largestSequenceBytes) + ",";
  extraJson += "\"largestSequenceName\":\"" + escapeJsonString(stats.largestSequenceName) + "\"";
  sendApiResponse(200, true, "Sequence storage stats loaded.", extraJson);
}

void handleSeqDuplicate() {
  if (!checkWebAuth()) return;

  if (!server.hasArg("source") || !server.hasArg("target")) {
    sendApiResponse(400, false, "Missing source or target parameter.");
    return;
  }

  String sourceName = server.arg("source");
  String targetName = server.arg("target");
  if (!sequenceManager.isValidSequenceName(sourceName.c_str()) ||
      !sequenceManager.isValidSequenceName(targetName.c_str())) {
    sendApiResponse(400, false, "Invalid sequence name.");
    return;
  }

  bool success = sequenceManager.duplicateSequence(sourceName.c_str(), targetName.c_str());
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? String("Duplicated sequence: ") + sourceName + " -> " + targetName
                          : String("Failed to duplicate sequence: ") + sourceName + " -> " + targetName);
}

void handleSeqExport() {
  if (!checkWebAuth()) return;
  ...
  String json;
  String errorMessage;
  if (!sequenceManager.exportSequenceJson(name.c_str(), json, errorMessage)) {
    sendApiResponse(404, false, errorMessage);
    return;
  }
...
void handleSeqImport() {
  if (!checkWebAuth()) return;

  String payload = server.hasArg("plain") ? server.arg("plain") : server.arg("data");
  payload.trim();

  if (payload.length() == 0) {
    sendApiResponse(400, false, "Missing sequence JSON payload.");
    return;
  }

  String importedName;
  String errorMessage;
  bool success = sequenceManager.importSequenceJson(payload, importedName, errorMessage);
  if (!success) {
    int statusCode = errorMessage.indexOf("already exists") >= 0 ? 409 : 422;
    sendApiResponse(statusCode, false, errorMessage);
    return;
  }

  sendApiResponse(200,
                  true,
                  String("Imported sequence: ") + importedName,
                  "\"name\":\"" + escapeJsonString(importedName.c_str()) + "\"");
}

void handleSeqPlaylistAdd() {
  if (!checkWebAuth()) return;
  ...
  bool success = sequenceManager.playlistAdd(name.c_str());
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? String("Added to playlist: ") + name : String("Failed to add to playlist: ") + name);
}

void handleSeqPlaylistRemove() {
  if (!checkWebAuth()) return;

  int index = 0;
  if (!server.hasArg("index") || !safeParseInt(server.arg("index"), index, 1, MAX_PLAYLIST_ITEMS)) {
    sendApiResponse(400, false, "Missing or invalid playlist index.");
    return;
  }

  bool success = sequenceManager.playlistRemove((uint8_t)(index - 1));
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? String("Removed playlist item ") + index : String("Failed to remove playlist item ") + index);
}

void handleSeqPlaylistMove() {
  if (!checkWebAuth()) return;

  int fromIndex = 0;
  int toIndex = 0;
  if (!server.hasArg("from") || !server.hasArg("to") ||
      !safeParseInt(server.arg("from"), fromIndex, 1, MAX_PLAYLIST_ITEMS) ||
      !safeParseInt(server.arg("to"), toIndex, 1, MAX_PLAYLIST_ITEMS)) {
    sendApiResponse(400, false, "Missing or invalid playlist move parameters.");
    return;
  }

  bool success = sequenceManager.playlistMove((uint8_t)(fromIndex - 1), (uint8_t)(toIndex - 1));
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? String("Moved playlist item ") + fromIndex + " to " + toIndex
                          : String("Failed to move playlist item ") + fromIndex + " to " + toIndex);
}

void handleSeqPlaylistSave() {
  if (!checkWebAuth()) return;

  if (!server.hasArg("name")) {
    sendApiResponse(400, false, "Missing playlist name parameter.");
    return;
  }

  String playlistName = server.arg("name");
  if (!sequenceManager.isValidSequenceName(playlistName.c_str())) {
    sendApiResponse(400, false, "Invalid playlist name.");
    return;
  }

  bool success = sequenceManager.playlistSave(playlistName.c_str());
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? String("Saved playlist: ") + playlistName
                          : String("Failed to save playlist: ") + playlistName);
}

void handleSeqPlaylistLoad() {
  if (!checkWebAuth()) return;
  ...
  bool success = sequenceManager.playlistLoad(playlistName.c_str());
  sendApiResponse(success ? 200 : 404,
                  success,
                  success ? String("Loaded playlist: ") + playlistName
                          : String("Failed to load playlist: ") + playlistName);
}

void handleSeqPlaylistList() {
  if (!checkWebAuth()) return;

  const int MAX_LIST = 20;
  char names[MAX_LIST][MAX_SEQUENCE_NAME_LENGTH];
  int count = sequenceManager.listPlaylists(names, MAX_LIST);

  String json = "{\"ok\":true,\"message\":\"Saved playlists loaded.\",\"playlists\":[";
  for (int i = 0; i < count; i++) {
    if (i > 0) json += ",";
    json += "{\"name\":\"";
    json += escapeJsonString(names[i]);
...
void handleSeqPlaylistClear() {
  if (!checkWebAuth()) return;

  sequenceManager.playlistClear();
  sendApiResponse(200, true, "Playlist cleared.");
}

void handleSeqPlaylistPlay() {
  if (!checkWebAuth()) return;

  bool success = sequenceManager.playlistStart(false);
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? "Playlist started." : "Playlist could not be started.");
}

void handleSeqPlaylistLoop() {
  if (!checkWebAuth()) return;

  bool success = sequenceManager.playlistStart(true);
  sendApiResponse(success ? 200 : 409,
                  success,
                  success ? "Playlist loop started." : "Playlist loop could not be started.");
}
...
void handleSeqPlaylistGet() {
  ...
  String json = "{\"ok\":true,\"message\":\"Playlist loaded.\",\"sequences\":[";
...
void handleSeqMapList() {
  if (!checkWebAuth()) return;

  String json = "{\"ok\":true,\"message\":\"IR mappings loaded.\",\"mappings\":[";
  bool first = true;
  ...
  for (int i = 0; i < config.buttonCount; i++) {
    if (config.buttons[i].isConfigured) {
      ...
      json += "\",\"mapped\":";
      json += strlen(config.buttons[i].sequenceName) > 0 ? "true" : "false";
      json += "}";
    }
  }
...
void handleSeqMapSet() {
  if (!checkWebAuth()) return;

  if (!server.hasArg("button") || !server.hasArg("sequence")) {
    sendApiResponse(400, false, "Missing button or sequence parameter.");
    return;
  }

  String buttonName = server.arg("button");
  String seqName = server.arg("sequence");
  ...
  if (buttonIndex < 0) {
    sendApiResponse(404, false, "Button not found.");
    return;
  }
  ...
  sendApiResponse(200,
                  true,
                  String("Mapped button ") + buttonName + " to sequence " + seqName + ".",
                  "\"button\":\"" + escapeJsonString(buttonName.c_str()) + "\",\"sequence\":\"" +
                    escapeJsonString(seqName.c_str()) + "\"");
}

void handleSeqMapClear() {
  if (!checkWebAuth()) return;

  if (!server.hasArg("button")) {
    sendApiResponse(400, false, "Missing button parameter.");
    return;
  }
  ...
  sendApiResponse(200,
                  true,
                  String("Cleared mapping for button ") + buttonName + ".",
                  "\"button\":\"" + escapeJsonString(buttonName.c_str()) + "\"");
}
