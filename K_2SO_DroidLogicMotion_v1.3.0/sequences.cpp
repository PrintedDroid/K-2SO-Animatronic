...
bool writeTextFileWithBackup(const String& path, const String& content, String& errorMessage) {
  ...
  if (!file) {
    errorMessage = "Failed to open temporary file for writing.";
    return false;
  }
  ...
  if (file.print(content) != content.length()) {
    file.close();
    LittleFS.remove(tempPath);
    errorMessage = "Failed to write file contents.";
    return false;
  }
  ...
  if (hadExistingFile && !LittleFS.rename(path, backupPath)) {
    LittleFS.remove(tempPath);
    errorMessage = "Failed to protect existing file before replace.";
    return false;
  }
  ...
  if (!LittleFS.rename(tempPath, path)) {
    ...
    LittleFS.remove(tempPath);
    errorMessage = "Failed to replace destination file.";
    return false;
  }
...
bool SequenceManager::begin() {
  ...
  if (!ensurePlaylistDir()) {
    Serial.println(F("Could not create playlists directory."));
    return false;
  }
...
bool SequenceManager::formatStorage() {
  ...
  sdAvailable = ensureSequenceDir() && ensurePlaylistDir();
  if (!sdAvailable) {
    Serial.println(F("Could not recreate sequence storage directories after format."));
    return false;
  }
...
bool SequenceManager::ensurePlaylistDir() {
  if (!LittleFS.exists(PLAYLISTS_DIR)) {
    return LittleFS.mkdir(PLAYLISTS_DIR);
  }
  return true;
}
...
String SequenceManager::getPlaylistPath(const char* name) {
  return String(PLAYLISTS_DIR) + "/" + String(name) + ".pls";
}
...
bool SequenceManager::duplicateSequence(const char* sourceName, const char* targetName) {
  if (!sdAvailable) {
    return false;
  }

  if (!isValidSequenceName(sourceName) || !isValidSequenceName(targetName)) {
    Serial.println(F("Invalid sequence name."));
    return false;
  }

  if (!sequenceExists(sourceName)) {
    Serial.println(F("Source sequence not found."));
    return false;
  }

  if (sequenceExists(targetName)) {
    Serial.println(F("Target sequence already exists."));
    return false;
  }

  SequenceFrame* frames = nullptr;
  uint16_t frameCount = 0;
  if (!loadSequenceFromSD(sourceName, frames, frameCount)) {
    return false;
  }

  bool saved = saveSequenceToSD(targetName, frames, frameCount);
  delete[] frames;

  if (saved) {
    Serial.print(F("Duplicated sequence '"));
...
bool SequenceManager::verifyAllSequences(uint16_t& okCount, uint16_t& failCount, String& report) {
  okCount = 0;
  failCount = 0;
  report = "";

  if (!sdAvailable) {
    report = "Sequence storage is not available.";
    return false;
  }

  File dir = LittleFS.open(SEQUENCES_DIR);
  if (!dir) {
    report = "Could not open sequence directory.";
    return false;
  }
  ...
  while (entry) {
    if (!entry.isDirectory()) {
      ...
      if (filename.endsWith(".seq")) {
        ...
        String errorMessage;
        if (verifySequence(filename.c_str(), info, errorMessage)) {
          okCount++;
          report += "[OK] ";
        } else {
          failCount++;
...
bool SequenceManager::getStorageStats(SequenceStorageStats& stats) {
  ...
  File seqDir = LittleFS.open(SEQUENCES_DIR);
  if (seqDir) {
    File entry = seqDir.openNextFile();
    while (entry) {
      if (!entry.isDirectory()) {
        String filename = String(entry.name());
        int lastSlash = filename.lastIndexOf('/');
        ...
        if (filename.endsWith(".seq")) {
          ...
          if (fileSize > stats.largestSequenceBytes) {
            stats.largestSequenceBytes = fileSize;
            filename.remove(filename.length() - 4);
            strncpy(stats.largestSequenceName, filename.c_str(), MAX_SEQUENCE_NAME_LENGTH - 1);
            stats.largestSequenceName[MAX_SEQUENCE_NAME_LENGTH - 1] = '\0';
          }
        }
      }
      entry.close();
      entry = seqDir.openNextFile();
    }
    seqDir.close();
  }

  File playlistDir = LittleFS.open(PLAYLISTS_DIR);
  if (playlistDir) {
...
bool SequenceManager::playlistSave(const char* name) {
  if (!sdAvailable) {
    return false;
  }

  if (!isValidSequenceName(name)) {
    Serial.println(F("Invalid playlist name."));
    return false;
  }

  if (playback.playlist.count == 0) {
    Serial.println(F("Cannot save an empty playlist."));
    return false;
  }
  ...
  doc["name"] = name;
  doc["version"] = 1;
  doc["count"] = playback.playlist.count;
  doc["loop"] = playback.playlist.loop;

  JsonArray sequenceArray = doc.createNestedArray("sequences");
  for (uint8_t i = 0; i < playback.playlist.count; i++) {
    sequenceArray.add(playback.playlist.sequences[i]);
  }
  ...
  String errorMessage;
  if (!writeTextFileWithBackup(getPlaylistPath(name), json, errorMessage)) {
    Serial.println(errorMessage);
    return false;
  }

  Serial.print(F("Playlist saved as '"));
  Serial.print(name);
  Serial.println(F("'"));
  return true;
}

bool SequenceManager::playlistLoad(const char* name) {
  if (!sdAvailable) {
    return false;
  }
  ...
  File file = LittleFS.open(path, "r");
  if (!file) {
    Serial.println(F("Saved playlist not found."));
    return false;
  }
  ...
  file.close();
  if (error) {
    Serial.print(F("Playlist JSON parse error: "));
    Serial.println(error.c_str());
    return false;
  }

  JsonArrayConst sequenceArray = doc["sequences"].as<JsonArrayConst>();
  uint8_t count = doc["count"] | 0;
  if (sequenceArray.isNull() || count == 0 || count > MAX_PLAYLIST_ITEMS || sequenceArray.size() != count) {
    Serial.println(F("Invalid saved playlist format."));
    return false;
  }

  Playlist loadedPlaylist = {};
  loadedPlaylist.count = count;
  loadedPlaylist.currentIndex = 0;
  loadedPlaylist.loop = doc["loop"] | false;
  loadedPlaylist.active = false;

  uint8_t index = 0;
  for (JsonVariantConst item : sequenceArray) {
    const char* sequenceName = item | "";
    if (!isValidSequenceName(sequenceName) || !sequenceExists(sequenceName)) {
      Serial.print(F("Saved playlist references missing/invalid sequence: "));
      Serial.println(sequenceName);
      return false;
    }
    strncpy(loadedPlaylist.sequences[index], sequenceName, MAX_SEQUENCE_NAME_LENGTH - 1);
    loadedPlaylist.sequences[index][MAX_SEQUENCE_NAME_LENGTH - 1] = '\0';
    index++;
  }
  ...
  playback.playlist = loadedPlaylist;
  Serial.print(F("Loaded playlist '"));
  Serial.print(name);
  Serial.println(F("'"));
  return true;
}

int SequenceManager::listPlaylists(char names[][MAX_SEQUENCE_NAME_LENGTH], int maxCount) {
  if (!sdAvailable) {
    return 0;
  }

  File dir = LittleFS.open(PLAYLISTS_DIR);
  if (!dir) {
    return 0;
  }
