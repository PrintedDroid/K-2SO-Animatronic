...
<script>
    ...
    async function loadPlaylistStatus() {
        try {
            ...
            if (!data.sequences || data.sequences.length === 0) {
                infoDiv.innerHTML = 'No sequences in playlist';
                return;
            }
    ...
    async function loadIRMappings() {
        try {
            ...
            if (!data.mappings || data.mappings.length === 0) {
                listDiv.innerHTML = '<div class="empty-message">No IR mappings configured</div>';
                return;
            }
            ...
            data.mappings.forEach(mapping => {
                html += `
                    <div class="ir-mapping-item">
                        ...
                        <div class="ir-sequence-name">${escapeHtml(mapping.sequence)}</div>
